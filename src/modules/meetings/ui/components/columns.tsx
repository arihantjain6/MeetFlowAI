...
const statusColorMap = {
    ...
    ["active"]: "bg-blue-500/20 text-green-800 border-green-800/5",
    ["completed"]: "bg-emerald-500/20 text-emerald-800 border-emerald-800/5",
    ["processing"]: "bg-gray-300/20 text-blue-800 border-blue-800/5",
...
export const columns: ColumnDef<MeetingsGetMany[number]>[] = [
            ...
            <div className="flex flex-col gap-y-1">
                ...
                <div className="flex items-center gap-x-2">
                    <div className="flex items-center gap-x-1 text-muted-foreground">
                        ...
                        <span className="text-sm max-w-[200px] truncate capitalize">{row.original.agent.name}</span>
                    </div>
                    <GeneratedAvatar 
    ...
    { 
        ...
        cell: ({ row }) => {
            ...
            return (
                <Badge
                variant="outline"
                className={cn("capitalize [&>svg]:size-4 text-muted-foreground" , statusColorMap[row.original.status as keyof typeof statusIconMap])}
                >
    ...
    },
    {
        ...
        header: "Duration",
        cell: ({ row }) => {
            const duration = row.original.duration ? Number(row.original.duration) : 0;
            return (
                <Badge
                    variant="outline"
